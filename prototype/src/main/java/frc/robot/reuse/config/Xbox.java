...
import frc.robot.reuse.math.HerdVector;
...
public class Xbox
{
    ...
    private static final double DEADBAND = 0.2;  // TODO Seems like this could be hurting sensitivity!!!
    ...
    public double getAxis(AXIS axis)
    {
        ...
        {
            final double val = -getRawAxis(axis.get());
            return (val == -0) ? 0 : val;
        }
    ...
    {
        final double x = getX(side);
        final double y = getY(side);
    ...
    {
        final RumbleType r = (side == Hand.kLeft) ? RumbleType.kLeftRumble : RumbleType.kRightRumble;
