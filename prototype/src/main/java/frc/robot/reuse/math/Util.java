...
public final class Util
{
    ...
    {
        // The Java mod operator returns values from -max to max, so we need to add max and mod again
     ...
     */
    public static final double scaleToRange(double value, double inMin, double inMax, double outMin, double outMax)
    {
        // Subtract off the input min value, scale based on the in and out ranges, and add the output min value
     ...
     */
    public static final double scaleToRange(double value, double inMax, double outMax)
    {
        double clamp =  clampToRange(value, 0, inMax);
        return (clamp != 0.0) ? clamp * outMax / inMax : 0.0;
    }
}
